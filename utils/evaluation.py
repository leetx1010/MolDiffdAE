...
#import .jnk3_scorer as jnk3_scorer
#import .gsk3b_scorer as gsk3b_scorer
...
descriptor2d_names = list(calc.GetDescriptorNames())


def get_descriptors2d(mol):
    Chem.SanitizeMol(mol)
    rdkit_2d_desc = []
    calc = MoleculeDescriptors.MolecularDescriptorCalculator([x[0] for x in Descriptors._descList])
    ...
    return rdkit_2d_desc, header

def get_oracles(mol):
    mol = copy.deepcopy(mol)
    Chem.SanitizeMol(mol)
    oracle_desc = {}
    oracle_desc['DRD2'] = drd2_scorer.get_score(mol)
    oracle_desc['JNK3'] = jnk3_scorer.get_score(mol)
    oracle_desc['GSK3B'] = gsk3b_scorer.get_score(mol)
...
def get_descriptors(mol):
    ...
    ### 3D descriptors
    rdkit_desc3d = Descriptors3D.CalcMolDescriptors3D(mol)
    rdkit_desc.update(rdkit_desc3d)

    # Oracles
    #oracle_desc = get_oracles(mol)
    #rdkit_desc.update(oracle_desc)
