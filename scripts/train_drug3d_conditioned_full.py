...
from models.model import MolDiffCond
...
if __name__ == '__main__':
    ...
    parser.add_argument('--property_list', type=str, default='selected3d')
    ...
    config.model.node_dim = config.model.node_dim + len(selected_property_idx)
    config.model.edge_dim = config.model.edge_dim + len(selected_property_idx)
    ...
    if config.model.name == 'diffusion':
        model = MolDiffCond(
            ...
            num_cond=len(selected_property_idx)
        ).to(args.device)
    else:
    ...
    def train(it):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=config.train.use_amp):
            loss_dict = model.get_loss(
                ...
                c = batch.prop[:,selected_property_idx],
            )
        loss = loss_dict['loss']
