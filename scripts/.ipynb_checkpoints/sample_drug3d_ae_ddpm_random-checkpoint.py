...
def sample_from_embedding(emb, model, mode='template', n_graphs=10, max_size=None, guidance=None, bond_predictor=None, manipulate=None):
    if max_size is None:
...
if __name__ == '__main__':
    ...
    # regression 
    with open(f'{args.logdir}/classifiers/linear_all.pkl', 'rb') as f:
    ...
    for i in range(50):
        manipulate = None
        emb = torch.randn(1, model.emb_dim)
        max_size = reg_dict['HeavyAtomCount'].predict(emb.numpy())[0]
        emb = emb.to(device)
        
        mol_list_manipulate = sample_from_embedding(emb, model, max_size=max_size, mode=args.template, n_graphs=n_graphs, manipulate=manipulate)
        ...
        emb_dict[i] = emb.detach().cpu().numpy()

    save_name = f'random-{args.name}_ddpm-{args.seed}'  
    with open(f'{args.logdir}/{save_name}.pkl', 'wb') as f:
        pickle.dump(gen_dict, f)

    with open(f'{args.logdir}/{save_name}_embeddings.pkl', 'wb') as f:
