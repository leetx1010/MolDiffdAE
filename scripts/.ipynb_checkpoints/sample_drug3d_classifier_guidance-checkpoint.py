...
if __name__ == '__main__':
    ...
    print("Using classifier models from", args.cls_dataset)
    if args.cls_dataset == 'crossdocked':
    ...
    if args.noise == 'deterministic':
        save_name = f'property-{args.name}_{args.task}_ddim-guidance-{args.cls_dataset}-{args.start_step}-{args.guidance_strength}'
        n_graphs = 1
        
    elif args.noise == 'random':
        save_name = f'property-{args.name}_{args.task}_ddim-guidance-randnoise-{args.cls_dataset}-{args.start_step}-{args.guidance_strength}'
        n_graphs = 10
    ...
    #for i in range(len(test_set)):
        if args.delta:
            prop = torch.tensor([[targets[n]]]).to(args.device)
            print(targets[n])
        else:
        ...
        mol_list_manipulate = reconstruct_ddim(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, prop=prop, guidance=guidance, predictor=predictor, start_step=args.start_step, with_edge=args.with_edge, n_graphs=n_graphs)
