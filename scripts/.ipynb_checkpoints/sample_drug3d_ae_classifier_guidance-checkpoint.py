...
def reverse_map_noise(data, model, stride=1, n_graphs=1, max_size=None, start_step=None):
    ...
    return None, h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, emb
    
def reconstruct_ddim(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, emb, prop, stride=1, n_graphs=1, start_step=None, with_edge=False, max_size=None, add_edge=None, guidance=None, predictor=None):
    if start_step is None:
        time_sequence = list(range(0, model.num_timesteps-stride, stride))
    else:
        time_sequence = list(range(0, start_step, stride))
...
if __name__ == '__main__':
    ...
    prop = torch.tensor([[target]]).to(args.device)
    ...
    #for i in idx_test[0:1]:
        print(i)
        """
        if i in [10623,25868] and args.start_step == 500 and args.task == 'fr_halogen+': # This will stuck, for unknown reasons
            gen_dict[i] = ([],[])
            continue
        if i in [10051,25465] and args.start_step == 999 and args.task == 'fr_halogen+': # This will stuck, for unknown reasons
            gen_dict[i] = ([],[])
            continue
        """
        
        _ , h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, emb = reverse_map_noise(test_set[i], model, start_step=args.start_step)
        print(h_halfedge_pert.shape)
        mol_list_manipulate = reconstruct_ddim(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, emb, prop=prop, guidance=guidance, predictor=predictor, start_step=args.start_step, with_edge=args.with_edge)
    ...
    save_name = f'property-{args.name}_{args.task}_ddim-guidance-{args.start_step}-{args.guidance_strength}'
    if args.with_edge:
        save_name += '-withedge'
