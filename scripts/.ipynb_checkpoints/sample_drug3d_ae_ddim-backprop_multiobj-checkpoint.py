...
def embedding_optimization(emb, aux_cls, manipulate):
    ...
    mode = manipulate[0][0]
    steps = manipulate[0][-1] 

    emb_prev = emb.to(emb.device)
    idx_pp_all = [manipulate[0][3], manipulate[1][3]]
    
    for i in range(steps):
        emb_in = emb_prev.detach().requires_grad_(True)

        for mode, target, alpha, idx_pp, steps in manipulate:
            pred_prop = aux_cls(emb_in)[:, idx_pp]
            
            if mode == 'mse':
                target = torch.tensor([target]).float().to(emb.device)
                loss_fn = torch.nn.MSELoss()
                loss = loss_fn(target, pred_prop)
                
            elif mode == 'value':
                loss = pred_prop.mean()
            emb_delta = -torch.autograd.grad(loss, emb_in, retain_graph=True)[0] * alpha

            emb_prev = emb_prev + emb_delta
    
    print('Optimized cls: ', aux_cls(emb_prev)[:, idx_pp_all].detach().cpu().numpy())
    return emb_prev.detach().cpu()

def sample_from_template_conditioned_multi_ddim(data, model, mode='template', noise='deterministic', stride=1, n_graphs=1, start_step=None, max_size=None, aux_cls=None, manipulate=None, add_edge=None):
    if max_size is None:
...
if __name__ == '__main__':
    ...
    config.dataset.split = 'split_by_molid.pt' # use the regular data split   
    ...
    manipulate = []
    for n, task in enumerate(task_list[args.task_id]):
        pp = task[:-1]
        idx_pp = selected_property_list.index(pp)

        if 'test' in args.property_set:
            alpha, _ = backprop_param_dict[pp]
            opt_steps = args.opt_steps
        else:
            alpha, opt_steps = args.alpha, args.opt_steps
        if args.mode == 'value' and args.task[-1] == '+':
            manipulate.append([args.mode, target[n], -alpha, idx_pp, opt_steps]) # gradient ascent
        else:
            manipulate.append([args.mode, target[n], alpha, idx_pp, opt_steps])

    # generation
    if args.noise == 'deterministic':
        save_name = f'property-{args.name}_multiobj-{args.task_id}_ddim-{args.start_step}-backprop-{args.mode}'
        n_graphs = 1
    elif args.noise == 'random':
        save_name = f'property-{args.name}_multiobj-{args.task_id}_ddim-randnoise-{args.start_step}-backprop-{args.mode}'
        n_graphs = 10
    
    gen_dict = {}
    for n, i in enumerate(idx_test):
        print(i)
        if args.task_id == 2 and i == 80: # this will get stuck
            gen_dict[i] = ([], [])
            continue
        if args.delta:
            for k in range(len(manipulate)):
                manipulate[k][1] = targets[n][k]
                
        mol_list_manipulate = sample_from_template_conditioned_multi_ddim(test_set[i], model, mode='template', n_graphs=n_graphs, manipulate=manipulate, start_step=args.start_step, aux_cls=model.encoder.aux_cls)
   ...
   # if 'test' in args.property_set:
    ...
    save_name += '_test'
    #else:
    #    save_name = f'property-{args.name}_multiobj-{args.task_id}_ddim-{args.start_step}-backprop-{args.mode}-{args.alpha}-{args.opt_steps}'
    #    if args.delta:
    #        save_name += '-delta'
