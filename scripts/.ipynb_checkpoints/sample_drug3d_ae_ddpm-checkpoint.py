...
if __name__ == '__main__':
    ...
    for n, i in enumerate(idx_test):
        if args.task not in ['random','orig']:
            ...
            manipulate = None
        mol_list_manipulate = sample_from_template_conditioned(test_set[i], model, mode=args.template, n_graphs=n_graphs, manipulate=manipulate, guidance=guidance, bond_predictor=bond_predictor)
    ...
    save_name = f'property-{args.name}_{args.task}_ddpm'
    if args.guidance is not None:
        save_name += '-guided'
