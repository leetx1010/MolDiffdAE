...
def sample_from_template_conditioned_ddim(data, model, mode='template', stride=1, n_graphs=1, start_step=None, max_size=None, guidance=None, bond_predictor=None, manipulate=None, add_edge=None):
    if max_size is None:
...
if __name__ == '__main__':
    ...
    for pp in descriptor_names:
        delta_dict[pp] = prop_std[descriptor_names.index(pp)] * args.delta
    ...
    for i in tqdm(range(len(test_set)//config.train.batch_size+1)):
    ...
    emb_list = np.array(torch.cat(emb_list))

    emb_train = emb_list[idx_train,:]
    emb_test = emb_list[idx_test,:]

    # If using external test set
    if args.name != 'drug3d':
        ...
        emb_test = np.array(torch.cat(emb_list))
    ...
    # regression    
    for delta in (delta_dict[pp], -delta_dict[pp]):
        if args.pp not in ['random','orig']:
            idx_pp = descriptor_names.index(args.pp)
            prop = prop_train[:,idx_pp]
            
            reg = LinearRegression().fit(emb_train, prop)
        ...
        #for i in idx_test[0:1]:
            target = prop_test[(idx_test==i), idx_pp] + delta
            manipulate = (reg, target, 1)
            
            mol_list_manipulate = sample_from_template_conditioned_ddim(test_set[i], model, mode=mode, n_graphs=n_graphs, manipulate=manipulate, start_step=args.start_step, bond_predictor=bond_predictor, guidance=guidance)
        ...
        if delta > 0:
            sign = f'increase-{args.delta}std'
        else:
            sign = f'decrease-{args.delta}std'
            
        if args.name == 'drug3d':
            save_name = f'property_{args.pp}_{sign}_ddim-{args.start_step}'
        else:
            save_name = f'property-{args.name}_{args.pp}_{sign}_ddim-{args.start_step}'
        if args.guidance is not None:
            save_name += '-guided'
