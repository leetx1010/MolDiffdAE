...
def embedding_optimization(emb, aux_cls, manipulate):
    ...
    print('Optimized cls: ', aux_cls(emb_prev)[:, idx_pp].item())
    return emb_prev.detach().cpu()

def sample_from_template_conditioned_ddim(data, model, mode='template', noise='deterministic', stride=1, n_graphs=1, start_step=None, max_size=None, aux_cls=None, manipulate=None, add_edge=None):
    if max_size is None:
...
if __name__ == '__main__':
    ...
    if args.noise == 'deterministic':
        save_name = f'property-{args.name}_{args.task}_ddim-{args.start_step}-backprop-{args.mode}-{args.opt_steps}'
        n_graphs = 1
    elif args.noise == 'random':
        save_name = f'property-{args.name}_{args.task}_ddim-randnoise-{args.start_step}-backprop-{args.mode}-{args.opt_steps}'
        n_graphs = 10
    ...
    for n, i in enumerate(idx_test):
        if args.task not in ['random','orig']:
            if args.delta:
                mode, target, alpha, idx_pp, opt_steps = manipulate
                manipulate = (mode, targets[n], alpha, idx_pp, opt_steps)
            print(manipulate[1])
        else:
            manipulate = None
