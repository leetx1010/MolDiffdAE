...
def reconstruct_ddim(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, prop, stride=1, n_graphs=1, start_step=None, max_size=None, add_edge=None, guidance=None, predictor=None):
    if start_step is None:
        time_sequence = list(range(0, model.num_timesteps-stride, stride))
    else:
        time_sequence = list(range(0, start_step, stride))
...
if __name__ == '__main__':
    ...
    for delta in (delta_dict[pp], -delta_dict[pp]):
        ckpt = torch.load(f'/home/mli/tili/mnt/MolDiffAE/model/property_predictor/PropertyPred-{idx_pp}/checkpoints/10000.pt', map_location='cuda')
        ...
        #for i in idx_test[0:1]:
            prop = torch.tensor([prop_test[(idx_test==i), idx_pp] + delta]).to(args.device)
            ...
            mol_list_manipulate = reconstruct_ddim(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, prop=prop, guidance=guidance, predictor=predictor, start_step=args.start_step)
        ...
        if args.name == 'drug3d':
            save_name = f'property_{args.pp}_{sign}_ddim-guidance-{args.start_step}-{args.guidance_strength}'
        else:
            save_name = f'property-{args.name}_{args.pp}_{sign}_ddim-guidance-{args.start_step}-{args.guidance_strength}'
