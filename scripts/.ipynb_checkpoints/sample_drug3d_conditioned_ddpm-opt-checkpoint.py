...
def reverse_map_noise(data, model, c, stride=1, n_graphs=1, start_step=None, max_size=None, pp=0):
    if max_size is None:
...
def reconstruct_ddpm(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, c, stride=1, n_graphs=1, start_step=None, max_size=None, add_edge=None):
    time_sequence = list(range(0, model.num_timesteps-stride, stride))
    edge_index = torch.cat([halfedge_index, halfedge_index.flip(0)], dim=1)
    batch_edge = torch.cat([batch_halfedge, batch_halfedge], dim=0)
...
if __name__ == '__main__':
    ...
    #for i in range(len(test_set)):
        ...
        _ , h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge = reverse_map_noise(test_set[i], model, c, pp=idx_pp, start_step=args.start_step)
        mol_list_manipulate = reconstruct_ddpm(h_node_pert, pos_pert, batch_node, h_halfedge_pert, halfedge_index, batch_halfedge, c_new, start_step=args.start_step)
    ...
    save_name = f'property-{args.name}_{args.task}_cddpm-opt-{args.start_step}'
    if args.delta:
        save_name += '-delta'
