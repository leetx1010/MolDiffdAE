...
class ContigousTransition(nn.Module):
    ...
    def reverse_sample_ddim(self, x_t, x_recon, t, s, batch):
        alpha_bar_t = extract(self.alphas_bar, t, batch)
        alpha_bar_s = extract(self.alphas_bar, s, batch)

        x_s = alpha_bar_s.sqrt() * x_recon - (1 - alpha_bar_s).sqrt() * (alpha_bar_t.sqrt() * x_recon - x_t) / (1 - alpha_bar_t).sqrt()
        return x_s
    
    def get_prev_from_recon_ddim(self, x_t, x_recon, t, batch):    
        coef_x0 = extract(self.coef_x0, t, batch)
        coef_xt = extract(self.coef_xt, t, batch)
