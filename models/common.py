...
class MLPResidualAdd(nn.Module):
    """
    MLP with the same hidden dim across all layers plus skip connections. 
    Only works for 2 layers temporarily.
    Notice hidden dim should be the same as in dim in this scenario.
    Adds the residuals
    """

    def __init__(self, in_dim, out_dim, num_layer=2, norm=True, act_fn='relu', act_last=False):
        super().__init__()
        
        self.layer1 = nn.Linear(in_dim, in_dim)
        self.layer2 = nn.Linear(in_dim, out_dim)
        self.layernorm = nn.LayerNorm(in_dim)
        self.nonlinearity = NONLINEARITIES[act_fn]
    ...
    def forward(self, x):
        ...
        #x1 = x1 + residual
        return self.layer2(x1 + residual)

class MLPResidualCat(nn.Module):
    """
    MLP with the same hidden dim across all layers plus skip connections. 
    Only works for 2 layers temporarily.
    Notice hidden dim should be the same as in dim in this scenario.
    Concatenates the residuals
    """
    ...
    def __init__(self, in_dim, out_dim, hidden_dim, num_layer=2, norm=True, act_fn='relu', act_last=False):
        ...
        self.layer2 = nn.Linear(in_dim + hidden_dim, out_dim)
    ...
    def forward(self, x):
        ...
        x1 = self.nonlinearity(x1)
        #x1 = x1 + residual
        return self.layer2(torch.cat((x1,residual), 1))
