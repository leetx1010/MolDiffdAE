...
class MolDiffCond(Module):
    def __init__(self,
        ...
        **kwargs
        ...
        edge_dim = config.edge_dim
        time_dim = config.diff.time_dim
        self.node_embedder = nn.Linear(num_node_types, node_dim-time_dim-num_cond, bias=False)  # element type
        self.edge_embedder = nn.Linear(num_edge_types, edge_dim-time_dim-num_cond, bias=False) # bond type
        self.time_emb = nn.Sequential(
            GaussianSmearing(stop=self.num_timesteps, num_gaussians=time_dim, type_='linear'),
        )
    ...
    def forward(self, h_node_pert, pos_pert, batch_node,
                h_edge_pert, edge_index, batch_edge, t, c):
        """
        Predict Mol at step `0` given perturbed Mol at step `t` with hidden dims and time step
        """
        # 1 node and edge embedding + time embedding
        time_embed_node = self.time_emb(t.index_select(0, batch_node))
        c_node = c.index_select(0, batch_node)
        h_node_pert = torch.cat([self.node_embedder(h_node_pert), time_embed_node, c_node], dim=-1)
        
        time_embed_edge = self.time_emb(t.index_select(0, batch_edge))
        c_edge = c.index_select(0, batch_edge)
        h_edge_pert = torch.cat([self.edge_embedder(h_edge_pert), time_embed_edge, c_edge], dim=-1)
    ...
    @torch.no_grad()
    def sample(self, n_graphs, batch_node, halfedge_index, batch_halfedge, bond_predictor=None, guidance=None, c=None):
        device = batch_node.device
        # # 1. get the init values (position, node types)
        # n_graphs = len(n_nodes_list)
        n_nodes_all = len(batch_node)
        ...
        if c is None:
            c = torch.randn(n_graphs, self.num_cond, device=device)
